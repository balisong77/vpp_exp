...
static u8 *format_protocol2_trace(u8 *s, va_list *args) {
...
vlib_node_registration_t protocol2_node;

#endif /* CLIB_MARCH_VARIANT */

#define foreach_protocol2_error                                                \
  _(PROCESSED, "protocol2 error processed packets")

typedef enum {
#define _(sym, str) PROTOCOL2_ERROR_##sym,
  foreach_protocol2_error
#undef _
      PROTOCOL2_N_ERROR,
} protocol2_error_t;

#ifndef CLIB_MARCH_VARIANT
static char *protocol2_error_strings[] = {
...
VLIB_NODE_FN(protocol2_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame) {
  u32 n_left_from, *from, *to_next;
  protocol2_next_t next_index;
    ...
    while (n_left_from >= 4 && n_left_to_next >= 2) {
      ...
      if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
        if (b0->flags & VLIB_BUFFER_IS_TRACED) {
          protocol2_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
          t->next_index = next0;
        ...
        if (b1->flags & VLIB_BUFFER_IS_TRACED) {
          protocol2_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
          t->next_index = next1;
  ...
  vlib_node_increment_counter(vm, protocol2_node.index,
                              PROTOCOL2_ERROR_PROCESSED, pkts_processed);
  return frame->n_vectors;
}

/* *INDENT-OFF* */
#ifndef CLIB_MARCH_VARIANT
VLIB_REGISTER_NODE(protocol2_node) = {
    .name = "protocol2",
    .vector_size = sizeof(u32),
    .format_trace = format_protocol2_trace,
    .type = VLIB_NODE_TYPE_INTERNAL,

    .n_errors = ARRAY_LEN(protocol2_error_strings),
    .error_strings = protocol2_error_strings,
