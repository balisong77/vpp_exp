...
if [[ "${config}" == "policy" ]]; then
    ...
    sudo "${vppctl_binary}" -s "${SOCKFILE}" ipsec sa add 2000 spi 2000 esp tunnel src 192.161.0.1 dst 192.162.0.1 crypto-alg "${crypto_alg}" crypto-key "${crypto_algkey}" salt 0x12345678
    ...
    sudo "${vppctl_binary}" -s "${SOCKFILE}" ip route add 192.161.0.0/16 via 10.12.0.3 ${intf}
